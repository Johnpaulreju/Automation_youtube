...
def download_video(url, filename="video.mp4"):
    """
    Downloads a YouTube video with a fixed filename, always overwriting the previous file.
    """
    ydl_opts = {
        'format': 'bestvideo[height<=720]+bestaudio/best[height<=720]',  # Ensures max 720p quality
        'outtmpl': filename,  # Always saves as 'video.mp4'
        'merge_output_format': 'mp4',  # Ensures output is MP4
        'noprogress': True,  # Disable progress output
        'quiet': True  # Reduce terminal output
    }

    # Remove the previous file if it exists (ensures overwrite)
...
def main():
    ...
    if schedule_option == 'yes' or schedule_option == 'y':
